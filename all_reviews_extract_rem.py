...
oldlink = True
locs = random.randint(15,25)
target =50
r = random.randint(1,200)
#links  = hotels.find({  'first_review_reached':{'$exists':False} ,'review_count':{'$gt':75}, '$or':[ {'scraper_counter':{'$exists':False} }, {'scraper_counter':{'$lt':49} }  ] },{ '_id':1, 'review_ids':1, 'review_count':1, 'scraper_counter':1}).limit(locs).skip(r)
links  = hotels.find({  'first_review_reached':{'$exists':False} ,'$or':[ {'scraper_counter':{'$exists':False} }, {'scraper_counter':{'$lt':49} }  ] ,'$or':[{ 'review_ids':{'$size':50}}, { 'review_ids':{'$size':51}}, { 'review_ids':{'$size':52}},{ 'review_ids':{'$size':53}} , { 'review_ids':{'$size':54}}] },{ '_id':1, 'review_ids':1, 'review_count':1, 'scraper_counter':1}).limit(locs).skip(r)
print links
addedlist =''
locations =[]
f_r_r ={}
